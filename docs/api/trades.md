...
Khi trade sang `confirmed`/`self_reported` → tạo **đúng 1** `price_record` (đơn giá); listing (nếu có) sang `closed` — tin gom KHÔNG tự đóng (chủ tin gom từ nhiều người bán).
