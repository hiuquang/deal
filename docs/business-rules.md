...
13. **Catalog ưu tiên, tự thêm khi thiếu (nới từ 0.12.1)** — user chọn thẻ qua autocomplete từ bảng `cards`; khi tên gõ vào không khớp chính xác thẻ nào, được đăng ký entry mới qua `POST /api/cards` (find-or-create theo game + tên + category, dedupe ở DB) — **áp dụng cho MỌI game** (quyết định chủ web 2026-07-20; trước đó chỉ mục その他). Entry tự thêm của pokemon/onepiece mang `setCode CUSTOM/CUSTOM-BOX` — tách khỏi catalog chuẩn, lọc được khi làm sạch dataset giá cho AI (dataset lõi ưu tiên entry catalog; entry CUSTOM chất lượng thấp hơn, chấp nhận đánh đổi để không chặn đăng bán khi catalog thiếu). `condition` vẫn bắt buộc và phải khớp `category` (`400 CONDITION_MISMATCH`).
