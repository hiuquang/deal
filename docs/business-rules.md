...
8. **Auto-close 7 ngày** — trade pending không được phản hồi → tự chốt `self_reported`, lazy-check khi có request đọc trade/price (không cron).
9. **1 listing chỉ có 1 trade chưa-cancelled** (`409 TRADE_EXISTS`). Listing sang `in_trade` khi có trade pending, `closed` khi chốt, mở lại `active` khi trade cancel.
