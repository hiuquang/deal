...
/**
 * Phần tương tác của trang tin bán. Dữ liệu ban đầu do server component cha
 * truyền xuống (`initial`) — không còn useEffect fetch lượt đầu, nên Google và
 * bộ quét link của Facebook/Zalo thấy nội dung thật thay vì spinner.
 * Mọi thao tác sau đó vẫn đi qua api-client và tự cập nhật state như cũ.
 */
export function ListingDetail({ initial }: { initial: ListingDto }) {
  const { me } = useAuth();
  const { t } = useI18n();
  const [listing, setListing] = useState<ListingDto>(initial);
  // actionError = lỗi hành động phụ (hủy tin, sửa giá) hiện inline — không
  // được làm bay nội dung đang xem. Lỗi TẢI trang giờ do server lo (404).
